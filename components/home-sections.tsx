...
import { ArrowRight, ArrowUpRight, ChevronLeft, ChevronRight, Eye, Leaf, MapPin, Maximize2, Minimize2, Target, Volume2, VolumeX } from 'lucide-react';
...
const brandData = [
  { id: 'wine', theme: 'wine', logo: 'icon-the-winee.png', title: <>THE WINE<br /><span className="card-brand-subtitle">Xperience</span></>, keywords: 'RÓTULOS  •  EXPERIÊNCIA  •  SOFISTICAÇÃO', description: 'Experiência voltada ao universo dos vinhos, com curadoria de rótulos e momentos para apreciar bons encontros.', images: ['The Wine Experience/salao-principal-clientes.webp', 'The Wine Experience/parede-vinhos-decoracao.webp', 'The Wine Experience/mesa-reservada-adega.webp', 'The Wine Experience/jantar-harmonizado-brinde.webp'] },
  { id: 'super', theme: 'super', logo: 'icon-b-supermercadoo.png', title: <>+B Supermercados</>, keywords: 'VARIEDADE  •  ECONOMIA  •  CONVENIÊNCIA', description: 'Rede de supermercados com mix completo de alimentos e ofertas para a rotina das famílias.', images: ['Supermercado +B/00_CardHome/1-foto-supermercado.webp', 'Supermercado +B/00_CardHome/2-foto-supermercado.webp', 'Supermercado +B/03_Carnes/unid-plaza-foto-de-secao-de-carnes.webp', 'Supermercado +B/02_Hortifruti/unid-plaza-foto-de-hortifruti.webp'] },
  { id: 'farma', theme: 'farma', logo: 'icon-b-farmaa.png', title: <>+B Farma</>, keywords: 'SAÚDE  •  BELEZA  •  CONVENIÊNCIA', description: 'Drogarias integradas oferecendo amplo estoque de medicamentos, cuidados para a pele e atendimento humanizado.', images: ['Mais B Farma/foto-farma-b (1).webp', 'Mais B Farma/foto-farma-b (2).webp', 'Mais B Farma/foto-farma-b (3).webp', 'Mais B Farma/foto-banner-maisb-farmacia.webp'] },
  { id: 'park', theme: 'park', logo: 'icon-vila-plaza.png', title: <>Villa Plaza Park</>, keywords: 'DIVERSÃO  •  FAMÍLIA  •  LAZER', description: 'Maior parque infantil de Belém, pensado para crianças e famílias, com atrações e segurança.', images: ['Villa Plaza (Park Infantil)/villa-plaza-park-tapana.webp', 'Villa Plaza (Park Infantil)/foto-park-infantil (1).webp', 'Villa Plaza (Park Infantil)/foto-park-infantil (2).webp', 'Villa Plaza (Park Infantil)/foto-park-infantil (3).webp'] },
  { id: 'plaza', theme: 'plaza', logo: 'icon-vila-plaza.png', title: <>Villa Plaza<br />Restaurante</>, keywords: 'SABOR  •  ENCONTROS  •  EXPERIÊNCIA', description: 'Ambiente acolhedor e cardápio pensado para momentos especiais, encontros e boa gastronomia.', images: ['Villa Plaza (Restaurante)/vila-plaza-restaurante-ambiente.webp', 'Villa Plaza (Restaurante)/vila-plaza-gastronomia_buffet_saladas_selecao.webp', 'Villa Plaza (Restaurante)/vila-plaza-gastronomia_buffet_pratos_quentes.webp', 'Villa Plaza (Restaurante)/vila-plaza-experiencia_servico_garcom.webp'] },
];
...
export function BrandsSection() {
  ...
  return <section className="brands-scroll-container" id="marcas"><div className="brands-sticky-wrapper"><div className="brands-black-block" onMouseMove={onBrandsMouseMove}><div className="brands-grid-overlay" /><div className="container brands-section-container"><div className="brands-section-head"><span className="eyebrow-gold">NOSSAS MARCAS</span><h2 className="brands-headline-title">Marcas que conectam, cuidam e fazem parte<br /><span className="headline-highlight">do dia a dia.</span></h2><div className="brands-subtitle-badge-wrapper"><p className="brands-subtitle-text">Cada marca conecta consumo, cuidado, gastronomia, lazer e experiências para o dia a dia.</p></div></div><div className="brands-carousel-stage" tabIndex={0} onKeyDown={onStageKeyDown} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} aria-label="Carrossel de marcas" role="region"><div className="brands-layout-split"><div className="brands-layout-left"><BrandOrbit active={active} onSelect={selectBrand} /></div><div className="brands-layout-right"><button type="button" className="brands-nav-arrow arrow-left" aria-label="Marca anterior" onClick={() => selectRelative(-1)}><ChevronLeft size={22} /></button><button type="button" className="brands-nav-arrow arrow-right" aria-label="Próxima marca" onClick={() => selectRelative(1)}><ChevronRight size={22} /></button>{brandData.map((brand) => <div className={'brand-detail-card theme-' + brand.theme + (brand.id === active ? ' active' : '')} data-brand={brand.id} id={'marca-' + brand.id} key={brand.id}><div className="card-top-gallery">{brand.images.map((image) => <div className="gallery-panel panel-image" key={image}><img src={asset('Fotografias/' + image)} alt="" /></div>)}</div><div className="card-details-body"><div className="card-logo-overlap"><img src={asset('Icones Submarcas/' + brand.logo)} alt={brand.id} /></div><div className="card-header-info"><span className="card-badge">MARCA EM DESTAQUE</span><h3 className="card-brand-title">{brand.title}</h3><span className="card-brand-keywords">{brand.keywords}</span><p className="card-brand-description">{brand.description}</p><div className="card-actions"><a href="#links" className="btn-card-primary">Ver links <ArrowUpRight size={15} aria-hidden="true" /></a><a href="#store-locator-teaser" className="open-map-trigger btn-card-secondary"><MapPin size={15} aria-hidden="true" /> Ver localização</a></div></div></div></div>)}</div></div></div></div></div></div></section>;
}
